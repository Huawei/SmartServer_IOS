...
+ (BOOL)judueIPhonePlatformSupportTouchID
{
    ...
    if(IS_Phone)
    {
        ...
        {
            NSArray *temp2arr = [[self platform] componentsSeparatedByString:@","];
            if (temp2arr.count > 1) {
                NSString *tempstr1 = [temp2arr firstObject];
                if (tempstr1.length > 6) {
                    NSString * numberPlatformStr = [tempstr1 substringWithRange:NSMakeRange(6, tempstr1.length-6)];
