...
@interface SafePwdChangeViewController ()
...
@property (assign, nonatomic) BOOL isChangeToNew;

@end

@implementation SafePwdChangeViewController {
    BOOL isSecond;
...
- (void)viewDidLoad {
    ...
    self.title = LocalString(@"modifypwdlock");
    ...
    self.alertLabel.text = LocalString(@"pleaseenteroldpwd");
    [self createBackBtn];
...
- (IBAction)didBtnAction:(UIButton *)sender {
    ...
    self.pwd5Label.text = @"";
    self.pwd6Label.text = @"";
    if (isSecond && self.isChangeToNew) {
    ...
    } else if (isSecond && !self.isChangeToNew) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3* NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (success)
            {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    //成功操作
                    isSecond = YES;
                    if (!weakSelf.isChangeToNew) {
                        weakSelf.alertLabel.text = LocalString(@"pleaseenternewpwd");
                    }
