//
//  StartupSettingViewController.m
...
@interface StartupSettingViewController ()<UIScrollViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource>
...
@property (copy, nonatomic) NSString *bootSourceOverrideMode;

@end

@implementation StartupSettingViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = LocalString(@"devicedetailbootset");
    
    NSArray *titleArray = nil;
    
        titleArray = @[LocalString(@"bootmediium"),LocalString(@"bootmode")];
    
    UIButton *tempBtn = nil;
    for (int i = 0; i < titleArray.count; i++) {
        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = CGRectMake((ScreenWidth/2)*i, 0,ScreenWidth/2, 40);
        if (i == 0) {
            self.curBtn = btn;
    ...
    [self.collectionView registerNib:[UINib nibWithNibName:@"StartupSettingOneCell" bundle:nil] forCellWithReuseIdentifier:@"StartupSettingOneCell"];
    if (@available(iOS 11.0, *)) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    StartupSettingOneCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"StartupSettingOneCell" forIndexPath:indexPath];

    if (indexPath.row == 0) {
        cell.isFirst = YES;
        cell.isSupportStartMode = YES;
    } else {
        cell.isFirst = NO;
        cell.isSupportStartMode = self.isSupportStartMode;
    }
