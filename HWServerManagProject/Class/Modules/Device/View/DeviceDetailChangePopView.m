...
@interface DeviceDetailChangePopView()<UITableViewDelegate,UITableViewDataSource>

@property (strong, nonatomic) UITableView *tableView;

@end

@implementation DeviceDetailChangePopView
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    label.text = LocalString(@"devicedetailswitchdev");
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    label.textColor = NormalApp_TableView_Title_Style_Color;
    label.font = [UIFont systemFontOfSize:15];
    label.text = LocalString(@"devicedetailnoswitdev");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.titleLabel.text = [NSString stringWithFormat:@"%@(%@)",model.otherName,model.deviceName];
    cell.curRow = indexPath.row;
    __weak typeof(self) weakSelf = self;
    cell.didClearBlock = ^(NSInteger row) {
        DeviceModel *model1 = self.dataArray[row];
        [[HWGlobalData shared] deleteHistoryDevice:model1];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.didSelectDeviceBlock) {
        self.didSelectDeviceBlock(self.dataArray[indexPath.row]);
    }
