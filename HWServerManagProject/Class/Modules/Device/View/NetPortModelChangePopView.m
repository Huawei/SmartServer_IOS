...
@interface NetPortModelChangePopView()<UITableViewDelegate,UITableViewDataSource>

@property (strong, nonatomic) UITableView *tableView;

@end

@implementation NetPortModelChangePopView

- (void)awakeFromNib {
    [super awakeFromNib];
    self.frame = CGRectMake(0, 0, ScreenWidth, ScreenHeight);
    [self addTarget:self action:@selector(removeFromSuperview) forControlEvents:UIControlEventTouchUpInside];
    self.dataArray = @[LocalString(@"ns_port_port_mode_fixed"),LocalString(@"ns_port_port_mode_automatic")].mutableCopy;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    label.text = LocalString(@"ns_port_title_port_mode_selection");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.didSelectNetPortModelBlock) {
        self.didSelectNetPortModelBlock(self.dataArray[indexPath.row]);
    }
