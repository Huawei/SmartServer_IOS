...
@interface NetIpVersionChangePopView()<UITableViewDelegate,UITableViewDataSource>

@property (strong, nonatomic) UITableView *tableView;

@end

@implementation NetIpVersionChangePopView {
    NSArray *dataArray;
}

- (void)awakeFromNib {
    [super awakeFromNib];
    dataArray = @[@"IPv4",@"IPv6",@"IPv4+IPv6"];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    label.text = LocalString(@"ns_network_ip_version_selection_title");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.didSelectIpVersionBlock) {
        NSString *value = dataArray[indexPath.row];
        self.didSelectIpVersionBlock(value,self.popRow);
    }
